...
{
	BYTE houseNumber[1] = { 0 };
	houseNumber[0] = pAdd->num ;
	int sizeLenStreet = (int)strlen(pAdd->street);
	int sizeLenCity = (int)strlen(pAdd->city);

	if (fwrite(&houseNumber, sizeof(BYTE), 1, fp) != 1)
		return 0;

	if (!writeIntToFile(sizeLenStreet, fp, "error write street len"))
		return 0;
	if (!writeCharsToFile(pAdd->street,sizeLenStreet,fp, "error write street name\n"))
		return 0;
	if (!writeIntToFile(sizeLenCity, fp, "error write city len"))
		return 0;
	if (!writeCharsToFile(pAdd->city, sizeLenCity,fp, "error write city name\n"))
		return 0;
	return 1;
}
int readAddressFromFileCompressed(Address* pAdd, FILE* fp)
{
	...
	BYTE houseNumber[1];
	if (fread(&houseNumber, sizeof(BYTE),1, fp) != 1)
		return 0;
	pAdd->num = (int)houseNumber[0];

	int lenStreet , lenCity;
	if (!readIntFromFile(&lenStreet, fp, "error read street len"))
		return 0;

	pAdd->street = (char*)calloc(lenStreet+1,sizeof(char));
	if (!pAdd->street)
	{
		return 0;
	}
	if (!(readCharsFromFile(pAdd->street, lenStreet, fp,"error street name reading")))
		return 0;

	if (!readIntFromFile(&lenCity, fp, "error read city len"))
		return 0;
	pAdd->city = (char*)calloc(lenCity+1,sizeof(char));
	if (!pAdd->city)
	{
		return 0;
	}
	if (!(readCharsFromFile(pAdd->city, lenCity, fp, "error city name reading")))
		return 0;

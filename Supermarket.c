					...
					const char* customersFileName, int fileProcess)
{
	...
	if (loadSuperMarketFromFile(pMarket, fileName, customersFileName, fileProcess) == 1)
	{
...
int		addProduct(SuperMarket* pMarket)
{
	...
	if (pProd != NULL) //This barcode exist in stock
		...
		CHECK_RETURN_0(addNewProduct(pMarket, barcode));
	}
...
int		insertNewProductToList(LIST* pProductList, Product* pProduct)
{
	...
	while (pN != NULL)
	{
		...
		if (compRes > 0) {//found a place for new item, the next one bigger	
			...
			CHECK_RETURN_0(L_insert(pPrevNode, pProduct));
...
{
	Customer cust;
	CHECK_RETURN_0(initCustomer(&cust));
